...
export default function Admin() {
  ...
  async function sendInvoice(e) {
    ...
    await supabase.functions.invoke('send-email', {
      body: {
        to:      selected.email,
        subject: `Invoice from BuildFlow — $${amount.toLocaleString()}`,
        html: `
          <h2>You have a new invoice from BuildFlow</h2>
          <p><strong>Amount:</strong> $${amount.toLocaleString()}</p>
          <p><strong>Description:</strong> ${invoiceForm.description}</p>
          ${invoiceForm.due_date ? `<p><strong>Due Date:</strong> ${new Date(invoiceForm.due_date).toLocaleDateString()}</p>` : ''}
          <p>Log in to your BuildFlow account to view and pay this invoice.</p>
          <p><a href="https://buildflow-sand.vercel.app/billing">Pay Now</a></p>
        `,
      },
    })
      ...
      {showInvoice && (
        <Modal title={`Send Invoice to ${selected?.name}`} onClose={() => setShowInvoice(false)}>
          <form onSubmit={sendInvoice} className="space-y-4">
            ...
            <p className="text-xs text-charcoal-400">The contractor will receive an email with a payment link. Payment goes through Stripe automatically.</p>
